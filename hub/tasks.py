...
def enviar_email_de_bienvenida_task(self: Any, user_id: int) -> None:
    try:
        ...
        logger.info(
            f"Email de bienvenida enviado exitosamente a {user.email} (ID: {user_id})"
        )
        ...
        logger.critical(
            f"Error inesperado al enviar email de bienvenida: {str(e)}",
            exc_info=True,
...
def enviar_email_recuperarcion_contrasenia_task(
    ...
    try:
        ...
        logger.error(
            f"Usuario no encontrado para recuperación (ID: {user_id}): {str(e)}"
        )
        # No reintentar para este error
        raise
        ...
        logger.error(
            f"Error SMTP al enviar email de recuperación a {user_id}: {str(e)}"
        )
        raise self.retry(exc=e)
        ...
        logger.critical(
            f"Error inesperado en recuperación (ID: {user_id}): {str(e)}",
            exc_info=True,
